...
class data_summary_wizard(models.TransientModel):
    _name = 'blb_client.data_summary_wizard'
    _description = 'Data Summary Wizard'

    client_id = fields.Many2one('blb_client.blb_client', string='Client')

    #Send Details for Data Summary to Populate Window 
    #@api.multi
    def get_dashboard_view(self):
        #Get + Run Select function
        self.env["blb_client.data_summary"]._get_summary_select()
        #Return - videos Window
        view_id = self.env.ref('blb_client.datasummary_pivot').id
        domain = ""
        context =""
        return {
            ...
            'res_model':'blb_client.data_summary',
            'view_id': False,
...
class data_summary(models.Model):
    _name = 'blb_client.data_summary'
    _description = 'Data Summary'
    _order = 'id desc'

    blb_branch = fields.Many2one('blb_client.branches', string='Branch', readonly=True)
    ...
    def _get_summary_select(self):
        ...
        data_summary_recordset = self.env['blb_client.data_summary']
    ...
    def _remove_datarows(self):
        sql = "DELETE FROM blb_client_data_summary WHERE id > 0"
