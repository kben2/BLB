...
class blb_client(models.Model):
    ...
    old_registryfile_id = fields.Many2one('blb_client.blb_db2_tbl_registry', string='Old Registry File')
    ...
    coordinates_system = fields.Many2one('blb_layer.coordinates_system', string='Co-ordinates System')
    ...
    created_branch = fields.Many2one('blb_base.branches', string='Created at', readonly=True, compute="_compute_created_branch", store=True)
...
class legal_ownershippurchase(models.Model):
    ...
    file_no = fields.Many2one('blb_client.blb_client', string='Client', required=True)
...
class blb_db2_tbl_registry(models.Model):
    ...
    def update_blb_client_fields(self):
        ...
        all_clients_rec = self.env['blb_client.blb_client'].search([('file_no', '!=', '')])
        for client_rec in all_clients_rec:
            client_fileno = client_rec.file_no
            #Get equivalent in Old MIS
            oldmis_rec = self.env['blb_client.blb_db2_tbl_registry'].search([('file_no', '=', client_fileno)])
            if oldmis_rec:
                #Update missing fields
                #block_no
                if client_rec.block_no == False or client_rec.block_no == '':
                    client_rec.update({
                        'block_no': oldmis_rec.block_no 
                    })
                #plot_no
                if client_rec.plot_no == False or client_rec.plot_no == '':
                    client_rec.update({
                        'plot_no': oldmis_rec.plot_no
                    })
                #contact_details
                if client_rec.contact_details == False or client_rec.contact_details == '':
                    client_rec.update({
                        'plot_no': oldmis_rec.telephone
                    })
                # client_rec.update({
                #     'block_no': oldmis_rec.block_no, 
                #     'plot_no': oldmis_rec.plot_no, 
