...
class blb_deedprocessing(models.Model):
    ...
    def action_to_team_leadprocessing(self):
        ...
        deedprocessing_rec = self.env['blb_deedprocessing.blb_deedprocessing'].search([('plot_number', '=', self.plot_number),('block_number', '=', self.block_number)])
        client_rec = self.env['blb_client.blb_client'].search([('plot_no', '=', self.plot_number),('block_no', '=', self.block_number)])
        #raise ValidationError(_( str(deedprocessing_rec) + "==" + str(client_rec) ))
        if len(deedprocessing_rec) > 1 or len(client_rec) > 1:
            raise ValidationError(_('Block & Plot Number already assigned to another Client!'))
        #Send notification/sms/email
        #Get Team Lead Processing ----
        users_rec = self.env['res.users'].search([('groups_id', '=', self.env.ref("blb_base.blb_processing_lead").id)])
        user_rec = users_rec[1]
        user_id = user_rec.id
