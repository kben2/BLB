...
class blb_unit(models.Model):
    ...
    department_id = fields.Many2one('blb_base.blb_department', required=True, string='Department', ondelete='cascade')
    active = fields.Boolean(default=True)
...
class notifications(models.Model):
    ...
    def test_send_sms(self):
        #Send SMS
        username = 'kbdeveloper'
        password = 'Developer_45335'
        response = ''
        #url = 'http://boxuganda.com/api.php?user=' + username + '&password=' + password + '&sender=InfoSMS&message=Testing Msg for BLB&reciever=0702840005'
        url = 'https://sms.thepandoranetworks.com/API/send_sms/?message=Testing Msg for BLB&username=0705178503&password=0705178503&sender=BLB&message_category=bulk&message_type=info&number=0702840005'
    ...
    def _send_sms(self,user_id,addressed_to,notification_details):
        ...
        password = 'kayemba25'
        response = ''
        if addressed_to and addressed_to != '' and notification_details and notification_details != '':
            #url = 'http://boxuganda.com/api.php?user=' + str(username) + '&password=' + str(password) + '&sender=InfoSMS&message=' + str(notification_details) + '&reciever=' + str(addressed_to)   
            url = 'https://sms.thepandoranetworks.com/API/send_sms/?message=' + str(notification_details) + '&username=0705178503&password=0705178503&sender=BLB&message_category=bulk&message_type=info&number=' + str(addressed_to)
            #raise ValidationError(_(url))
    ...
    def _send_email(self,user_id,addressed_to,notification_subject,notification_details):        
        ...
        try:
            ...
            smtp_server.sendmail('BLB - ERP', addressed_to, email_body)
            smtp_server.close()
