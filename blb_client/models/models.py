...
class blb_client(models.Model):
    _name = 'blb_client.blb_client'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _description = 'BLB Clients'
    _order = 'file_no asc, location asc'
    ...
    old_registryfile_id = fields.Many2one('blb_client.blb_db2_tbl_registry', string='Old Registry File', ondelete='cascade')
    file_no = fields.Char(string="BLB File No.")
    client_name = fields.Char(string="Client Name", required=True)
    contact_details = fields.Char(string="Telephone Contact")
    contact_email = fields.Char(string="Email Contact")
    contact_details_2 = fields.Char(string="Other Telephone Contacts")
    other_contact_details = fields.Char(string="Other Contact Details")
    ...
    coordinates_system = fields.Many2one('blb_layer.coordinates_system', string='Co-ordinates System', ondelete='cascade')
    ...
    state = fields.Selection([
        ...
        ('issue', 'Title Issued'),
        ('done', 'Completed'),
        ('with_correspondence', 'With Correspondence'),
        ('cancel', 'Cancelled'),
    ...
    created_branch = fields.Many2one('blb_base.branches', string='Created at', readonly=True, compute="_compute_created_branch", store=True, ondelete='cascade')
    ...
    #Ownership
    plot_ownership_lines = fields.One2many('blb_client.plot_ownership', 'file_no', string='Owners')

    #Plot Coordinates
    plot_coordinates = fields.One2many('blb_client.plot_coordinates', 'file_no', string='Plot Co-ordinates')
    ...
    legal_ownershippurchase = fields.One2many('blb_client.legal_ownershippurchase', 'file_no', string='Purchase') #Documents Showing Acquistion/Ownership
    legal_ownershipinheritance = fields.One2many('blb_client.legal_ownershipinheritance', 'file_no', string='Inheritance')
    legal_ownershipdonation = fields.One2many('blb_client.legal_ownershipdonation', 'file_no', string='Donation')
    legal_companies = fields.One2many('blb_client.legal_companies', 'file_no', string='Companies')
    legal_ngos = fields.One2many('blb_client.legal_ngos', 'file_no', string='Non-Government Organisations')
    legal_trusts = fields.One2many('blb_client.legal_trusts', 'file_no', string='Trusts')
        ...
        for record in self:
            if record.location_x and record.location_y:
                if record.coordinates_system == False:
                    ...
                    coordinates_system_record = self.env['blb_layer.coordinates_system'].search([('id', '=', systemid)])
    ...
    def create(self, vals):
        ...
        return super(blb_client, self).create(vals)
    ...
    def _get_map_details(self):
        ...
        if self.id and self.file_no:
            for rec in self:
                ...
                #template = self.env.ref('blb_client.blb_clientmap_page_iframe')
    ...
    def get_clientmap(self,file_no):
        #Get Record
        client_rec = self.env['blb_client.blb_client'].search([('file_no', '=', file_no)])
    ...
    def view_map(self):
        ...
        if self.id:
            rec_id = self.id
            client_rec = self.env['blb_client.blb_client'].search([('id', '=', rec_id)])
            client_fileno = client_rec.file_no
        ...
        view_id = self.env.ref('blb_client.blb_clients_maplist_form').id
        domain = ""
        ...
        return {
            ...
            'view_type':'form',
            'view_mode':'form',
            'res_model':'blb_client.blb_client',
            'view_id': view_id,
...
class client_plots(models.Model):
    _name = 'blb_client.blb_client_plots'
...
class legal_ownershippurchase(models.Model):
    _name = 'blb_client.legal_ownershippurchase'
...
class legal_ownershipinheritance(models.Model):
    _name = 'blb_client.legal_ownershipinheritance'
...
class legal_ownershipdonation(models.Model):
    _name = 'blb_client.legal_ownershipdonation'
...
class legal_companies(models.Model):
    _name = 'blb_client.legal_companies'
...
class plot_ownership(models.Model):
    _name = 'blb_client.plot_ownership'
    ...
    file_no = fields.Many2one('blb_client.blb_client', string='Client', required=True, ondelete='restrict')


class plot_coordinates(models.Model):
    _name = 'blb_client.plot_coordinates'
...
class allclients_map(models.TransientModel):
    _name = 'blb_client.allclients_map'
...
## -----------
##  This Module Store all the data from the Old MIS and Links it to this New MIS
##  All fields are as they are the MYSQL - blb_db2.tbl_registry table
##  Imported to the Postgres Database with same names
## -----------------------------------------

class blb_db2_tbl_registry(models.Model):
    _name = 'blb_client.blb_db2_tbl_registry'
    _description = 'MySQL - Table Registry'
    _order = 'file_no asc'

    name = fields.Char(string="Old Client File - Details.", readonly=True, compute="_compute_name", store=True)
    ...
    if_other_specify_nationality = fields.Char(string="Other Nationality")
    purpose_of_land = fields.Char(string="Purpose of Land")
    if_other_specify_purpose = fields.Char(string="Other Purpose")
    nature_of_business = fields.Char(string="Business Nature")
    date_business_established = fields.Char(string="Date Business Established")
    director_trustee_1 = fields.Char(string="Director Trustee")
    share_holding_1 = fields.Char(string="Share holding 1")
    director_trustee_2 = fields.Char(string="Director Trsutee 2")
    share_holding_2 = fields.Char(string="Share Holding 2")
    director_trustee_3 = fields.Char(string="Director Trustee 3")
    share_holding_3 = fields.Char(string="Share holding 3")
    block_no = fields.Char(string="Block No.")
    plot_no = fields.Char(string="Plot No.")
    approximate_area = fields.Char(string="Approximate Area")
    district = fields.Char(string="District")
    county = fields.Char(string="County")
    sub_county = fields.Char(string="Subcounty")
    lc_zone = fields.Char(string="LC Zone")
    parish = fields.Char(string="Parish")
    value_of_project = fields.Char(string="Project value")
    type_of_tenure = fields.Char(string="Type of Tenure")
    details_of_tenure = fields.Char(string="Details of Tenure")
    development_activity = fields.Char(string="Development Activity")
    joint_ownership = fields.Char(string="Joint Ownership")
    tenants_in_common = fields.Char(string="Tenants Common")
    date_of_application = fields.Char(string="Date of Application")
    application_receipt_no = fields.Char(string="Receipt No.")
    application_fees = fields.Char(string="Application Fees")
    relationship_manager = fields.Char(string="Relationship Manager")
    recommenders_name = fields.Char(string="Recommenders Name")
    recommenders_profession = fields.Char(string="Recommenders Profession")
    recommenders_address = fields.Char(string="Recommenders Address")
    file_status = fields.Char(string="File Status")
    land_title = fields.Char(string="Land title")
    passport_photo = fields.Char(string="Passport Photo")
    land_house_photo = fields.Char(string="Land House Photo")
    ownership_documents = fields.Char(string="Ownership Documents")
    deed_print = fields.Char(string="Deed Print")
    lease_offer = fields.Char(string="Lease Offer")
    receipts = fields.Char(string="Receipts")
    lease_agreement = fields.Char(string="Lease Agrement")
    sketch_map = fields.Char(string="Sketch Map")
    other_documents = fields.Char(string="Other Documents")
    type_of_applicant = fields.Char(string="Type of Application")
    minute_number = fields.Char(string="Minute number")
    date_of_minute = fields.Char(string="File")
    duration = fields.Char(string="Duration")
    start_date = fields.Char(string="Start Date")
    kcc_start_date = fields.Char(string="KCC Start Date")
    premium = fields.Char(string="Premium")
    annual_rent = fields.Char(string="Annual rent")
    revision_period_from = fields.Char(string="Revision Period From")
    revision_period_after = fields.Char(string="Revision Period After")
    name_of_witness_and_address = fields.Char(string="Name of Witness Address")
    lease_status = fields.Char(string="Lease Status")
    volume_number = fields.Char(string="Volume Number")
    folio_number = fields.Char(string="Folio number")
    city = fields.Char(string="City")
    duration_of_term = fields.Char(string="Duration term")
    commencement_of_term = fields.Char(string="Commencement of Term")
    instrument_number = fields.Char(string="Instrument number")
    registration_date = fields.Char(string="Registration Date")
    date_of_certificate_issue = fields.Char(string="Date of Certificate Issue")
    remarks_on_file_disputes = fields.Char(string="Remarks on File Disputes")
    certificate_of_title_status = fields.Char(string="File")
    informational_note = fields.Char(string="Informational Note")
    migrated_flag = fields.Char(string="Migrated Flag")
    date_migration = fields.Char(string="Date Migration")
    file_tracker_flag = fields.Char(string="File tracker Flag")
    accessed_by = fields.Char(string="Accessed by")
    migrated_msdynamics = fields.Char(string="Migrated MS Dynamics")
    modified_status = fields.Char(string="Modified Status")
    date_migrated_msdy = fields.Char(string="Date Migrated MS Dynamics")
    file_cleaned = fields.Char(string="Flie Cleaned")
    last_updated = fields.Char(string="Last updated")
    le_card = fields.Char(string="Le-Card")

    #Compute Name
    @api.depends('file_name','file_no')
    def _compute_name(self):
        clientname = ""
        for rec in self:
            if rec.file_name and rec.file_no:
                clientname = str(rec.file_name) + " (" + str(rec.file_no) + ")"
            rec.name = clientname
        return clientname


    #Update Client Plot/Block/Contact/Location from Old LMIS Data
    def update_blb_client_fields(self):
        #Get All Rows - populate missing fields
        all_clients_rec = self.env['blb_client.blb_client'].search([('old_registryfile_id', '!=', False)])
        for client_rec in all_clients_rec:
            client_fileno = client_rec.file_no
            old_regfile_id = client_rec.old_registryfile_id.id
            #Get equivalent in Old MIS
            oldmis_rec = self.env['blb_client.blb_db2_tbl_registry'].search([('id', '=', old_regfile_id)])
            if oldmis_rec:
                #Update missing fields
                client_rec.update({
                    'block_no': oldmis_rec.block_no, 
                    'plot_no': oldmis_rec.plot_no, 
                    'contact_details': oldmis_rec.telephone 
                })